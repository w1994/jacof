...
public class GCDAgeExploration extends PseudoRandomProportionalRule {
    public GCDAgeExploration(ACO aco, AbstractAntSelection antSelection) {
    ...
    public int doExploration(Ant ant, int i) {
        ...
        for (Integer j : ant.getNodesToVisit()) {
            ...
            nij[j] = Math.pow(aco.getProblem().getNij((ScAnt) ant, i, j), ((ScAnt) ant).getBeta(aco.getNumberOfCurrentIteration(), aco.getNumberOfIterations()));
